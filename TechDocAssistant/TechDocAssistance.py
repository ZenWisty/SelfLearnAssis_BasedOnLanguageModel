...
class TutorialAssistant(Role):
    ...
    async def _act(self) -> Message:
        ...
        由于首先加入的 是 writeDirectory 类的action，因此在最开始的时候生成一次 directory,然后
        调用_handle_directory ，在_handle_directory 中将writeContent 类的action加入多次，然后
        rc.todo =None ，这样下次 的 think 会依次执行 WriteContent
        """
        todo = self.rc.todo
